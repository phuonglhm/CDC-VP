...
// Runtime Demo Testbench comparing PE parameter profiles:
...
public:
    NpuTop<EVAL_X, EVAL_Y, NPU_DTYPE_IN, NPU_DTYPE_IN, NPU_DTYPE_PSUM, A_REGION_BYTES, B_REGION_BYTES, C_REGION_BYTES, 16, EVAL_X + EVAL_Y, 1>* npu_std{nullptr};
    NpuTop<EVAL_X, EVAL_Y, NPU_DTYPE_IN, NPU_DTYPE_IN, NPU_DTYPE_PSUM, A_REGION_BYTES, B_REGION_BYTES, C_REGION_BYTES, 16, EVAL_X + EVAL_Y, 1>* npu_approx{nullptr};
    NpuTop<EVAL_X, EVAL_Y, NPU_DTYPE_IN, NPU_DTYPE_IN, NPU_DTYPE_PSUM, A_REGION_BYTES, B_REGION_BYTES, C_REGION_BYTES, 16, EVAL_X + EVAL_Y, 1>* npu_gated{nullptr};
...
private:
    ...
    const uint32_t SRAMA_REG_B_OFFSET = (416 * 1024) / (4 * sizeof(NPU_DTYPE_IN));
    const uint32_t SRAMB_BANK_1_OFFSET = (320 * 1024) / (4 * sizeof(NPU_DTYPE_IN));
    const uint32_t SRAMC_BANK_5_OFFSET = (50 * 1024) / (4 * sizeof(NPU_DTYPE_PSUM));
...
#define SAURIA_OUT_IDX_W 14
    ...
    std::vector<uint8_t> load_initial_dram_file(const std::string &path)
    {
        ...
        if (!file.is_open())
        {
            std::cerr << "[TB_DEMO ERROR] Cannot open initial DRAM file: "
                      << path << std::endl;
            return dram;
        }
        ...
        while (file >> token)
        {
            ...
            {
                std::cerr << "[TB_DEMO WARNING] Invalid DRAM token: "
                          << token << std::endl;
                continue;
            }
        ...
        std::cout << "[TB_DEMO] Loaded initial_dram.txt: "
                  << dram.size()
                  << " bytes"
                  << std::endl;
        ...
        for (uint32_t phys_addr = 0; loaded < nbytes; phys_addr++)
        {
            for (int sw = 0; sw < subwords_a && loaded < nbytes; sw++)
            {
                ...
                for (int p = 0; p < 3; p++)
                {
                    ...
                    write_mem(p, SRAMA_OFFSET | (SRAMA_REG_B_OFFSET + (get_srama_addr(phys_addr, sw) & ~SRAMA_OFFSET)), pkt);
                }
            }
        }
        ...
        for (uint32_t phys_addr = 0; loaded < nbytes; phys_addr++)
        {
            for (int sw = 0; sw < subwords_b && loaded < nbytes; sw++)
            {
                ...
                for (int p = 0; p < 3; p++)
                {
                    ...
                    write_mem(p, SRAMB_OFFSET | (SRAMB_BANK_1_OFFSET + (get_sramb_addr(phys_addr, sw) & ~SRAMB_OFFSET)), pkt);
                }
            }
        }
        ...
        for (uint32_t phys_addr = 0; elem_idx < total_elems; phys_addr++)
        {
            for (int sw = 0; sw < subwords_c && elem_idx < total_elems; sw++)
            {
                ...
                for (int p = 0; p < 3; p++)
                {
                    ...
                    write_mem(p, SRAMC_OFFSET | (SRAMC_BANK_5_OFFSET + (get_sramc_addr(phys_addr, sw) & ~SRAMC_OFFSET)), pkt);
                }
            }
        }
    ...
    void debug_readback_sram_regions()
    {
        ...
        std::cout << "SRAMA Region B:\n";
        for (int i = 0; i < 4; i++)
        {
            uint32_t addr = SRAMA_OFFSET | (SRAMA_REG_B_OFFSET + (get_srama_addr(i, 0) & ~SRAMA_OFFSET));
            host_data_t data = read_mem(0, addr);
            std::cout << "SRAMA Region B addr=0x" << std::hex << addr << std::dec << " : ";
            for (int j = 0; j < 4; j++)
            {
                std::cout << data[j] << " ";
            }
        ...
        std::cout << "SRAMB Bank 1:\n";
        for (int i = 0; i < 4; i++)
        {
            uint32_t addr = SRAMB_OFFSET | (SRAMB_BANK_1_OFFSET + (get_sramb_addr(i, 0) & ~SRAMB_OFFSET));
            host_data_t data = read_mem(0, addr);
            std::cout << "SRAMB Bank 1 addr=0x" << std::hex << addr << std::dec << " : ";
            for (int j = 0; j < 4; j++)
            {
                std::cout << data[j] << " ";
            }
        ...
        std::cout << "SRAMC Bank 5:\n";
        for (int i = 0; i < 4; i++)
        {
            uint32_t addr = SRAMC_OFFSET | (SRAMC_BANK_5_OFFSET + (get_sramc_addr(i, 0) & ~SRAMC_OFFSET));
            host_data_t data = read_mem(0, addr);
            std::cout << "SRAMC Bank 5 addr=0x" << std::hex << addr << std::dec << " : ";
            for (int j = 0; j < 4; j++)
            {
                std::cout << data[j] << " ";
            }
                              ...
                              uint32_t baseC)
    {
        ...
        std::vector<uint8_t> dram_std =
            load_initial_dram_file(demo_initial_dram_path());
        m_dram = dram_std;
        if (npu_std) npu_std->set_dram(&m_dram);
        if (npu_approx) npu_approx->set_dram(&m_dram);
        if (npu_gated) npu_gated->set_dram(&m_dram);
        std::vector<uint8_t> dram_approx = dram_std;
        std::vector<uint8_t> gold =
            load_initial_dram_file(demo_gold_dram_path());
        ...
        for (uint32_t tile = 0; tile < n_tiles; tile++)
        {
            ...
            o_mvm_k.write(mvm_k); g_demo_mvm_k = (long)mvm_k;
            ...
            for (uint32_t x = 0; x < cxlim; x++)
            {
                for (uint32_t ctx = 0; ctx < ncontexts; ctx++)
                {
                    ...
                    uint32_t nsplit = EVAL_Y / 2;
                    for (int sw = 0; sw < subwords_c; sw++)
                    {
                        host_data_t vs_a = read_mem(0, get_sramc_addr(base_elem / EVAL_Y, sw));
                        host_data_t va_a = read_mem(1, get_sramc_addr(base_elem / EVAL_Y, sw));

                        host_data_t vs_b = read_mem(0, SRAMC_OFFSET | (SRAMC_BANK_5_OFFSET + (get_sramc_addr(base_elem / EVAL_Y, sw) & ~SRAMC_OFFSET)));
                        host_data_t va_b = read_mem(1, SRAMC_OFFSET | (SRAMC_BANK_5_OFFSET + (get_sramc_addr(base_elem / EVAL_Y, sw) & ~SRAMC_OFFSET)));
                        ...
                        for (int lane = 0; lane < 4; lane++)
                        {
                            ...
                            int32_t cs = (y < nsplit) ? (int32_t)vs_a[lane] : (int32_t)vs_b[lane];
                            int32_t ca = (y < nsplit) ? (int32_t)va_a[lane] : (int32_t)va_b[lane];
    ...
    void test_process()
    {
        ...
        std::cout << "[TB_DEMO] Fetching initial_dram.txt into Memory Pre-load..." << std::endl;
        std::ifstream dram_file(demo_initial_dram_path());
        ...
        dram_file.close();
        std::cout << "[TB_DEMO] Completed parsing initial_dram.txt with " << dram_data.size() << " floats loaded." << std::endl;

        std::cout << "[TB_DEMO] Mapping data to internal SRAM A & B..." << std::endl;
        int byte_ptr = 0;
        ...
        std::cout << "[TB_DEMO] Completed memory pre-load from initial_dram.txt." << std::endl;
        std::cout << "[TB_DEMO] Input activations and weights programmed to double buffers." << std::endl;
        ...
        for (int p = 0; p < 3; p++)
        {
            ...
            std::cout << "[TB_DEMO] Profile " << p << " Readback SRAM A0: [" << read_act0[0] << ", " << read_act0[1] << ", " << read_act0[2] << ", " << read_act0[3] << "]" << std::endl;
            std::cout << "[TB_DEMO] Profile " << p << " Readback SRAM A1: [" << read_act1[0] << ", " << read_act1[1] << ", " << read_act1[2] << ", " << read_act1[3] << "]" << std::endl;
            std::cout << "[TB_DEMO] Profile " << p << " Readback SRAM B0: [" << read_wei0[0] << ", " << read_wei0[1] << ", " << read_wei0[2] << ", " << read_wei0[3] << "]" << std::endl;
            std::cout << "[TB_DEMO] Profile " << p << " Readback SRAM B1: [" << read_wei1[0] << ", " << read_wei1[1] << ", " << read_wei1[2] << ", " << read_wei1[3] << "]" << std::endl;
        }
        ...
        std::cout << "[TB_DEMO] Reading and execute instructions set from GoldenStimuli.txt..." << std::endl;
        std::ifstream stim_file(demo_golden_stimuli_path());
            ...
            if (is_controller_start_write(raw_addr, data_in, wren))
            {
                ...
                // ---------------------------------------------------------
                {
                    ...
                    std::vector<uint32_t> a_shape =
                        load_shape_u32(demo_sauria_path("sauria_A_Mat_mvm_shape.txt"));

                    std::vector<uint32_t> b_shape =
                        load_shape_u32(demo_sauria_path("sauria_B_Mat_mvm_shape.txt"));
                ...
                std::string initial_dram_path = demo_initial_dram_path();
                m_dram = load_initial_dram_file(initial_dram_path);
                if (npu_std) npu_std->set_dram(&m_dram);
                if (npu_approx) npu_approx->set_dram(&m_dram);
                if (npu_gated) npu_gated->set_dram(&m_dram);
                std::vector<uint8_t>& dram = m_dram;
        ...
        stim_file.close();
        std::cout << "[TB_DEMO] Executed " << command_count << " transactions from GoldenStimuli.txt" << std::endl;
        ...
        std::cout << "[TB_DEMO] Triggering NPU execution..." << std::endl;
        o_start_std.write(true);
        o_start_approx.write(true);
        o_start_gated.write(true);
        ...
        std::cout << "[TB_DEMO] Waiting for NPU to complete execution..." << std::endl;
        ...
        for (int cycle = 0; cycle < 20000; cycle++)
        {
            ...
            if (i_done_std.read())
            {
                ...
                std::cout << "[TB_DEMO] STD NPU done at cycle " << cycle << std::endl;
                break;
            }
        }
        ...
        if (!completed)
        {
            std::cout << "[TB_DEMO WARNING] STD NPU execution timeout." << std::endl;
        }
        ...
        {
            std::cerr << "[TB_DEMO ERROR] decoded_cfg_runtime is not valid. "
        ...
        {
            std::cerr << "[TB_DEMO ERROR] C output region out of range.\n"
                      << "  out_base          = " << out_base << "\n"
                      << "  total_c_elements  = " << total_c_elements << "\n"
        ...
        for (uint32_t out_tile = 0; out_tile < output_tiles; out_tile++)
        {
            for (uint32_t x = 0; x < cxlim; x++)
            {
                for (uint32_t ctx = 0; ctx < ncontexts; ctx++)
                {
                    ...
                    for (int sw = 0; sw < subwords_c; sw++)
                    {
                        host_data_t vs_a = read_mem(0, get_sramc_addr(base_elem_addr / EVAL_Y, sw));
                        host_data_t va_a = read_mem(1, get_sramc_addr(base_elem_addr / EVAL_Y, sw));

                        host_data_t vs_b = read_mem(0, SRAMC_OFFSET | (SRAMC_BANK_5_OFFSET + (get_sramc_addr(base_elem_addr / EVAL_Y, sw) & ~SRAMC_OFFSET)));
                        host_data_t va_b = read_mem(1, SRAMC_OFFSET | (SRAMC_BANK_5_OFFSET + (get_sramc_addr(base_elem_addr / EVAL_Y, sw) & ~SRAMC_OFFSET)));
                        ...
                        for (int lane = 0; lane < 4; lane++)
                        {
                            ...
                            double chunk_std_val = (y < nsplit) ? vs_a[lane] : vs_b[lane];
                            double chunk_approx_val = (y < nsplit) ? va_a[lane] : va_b[lane];
                            ...
                            float compute_std =
                                row_active ? static_cast<float>(chunk_std_val) : 0.0f;
                            float compute_approx =
                                row_active ? static_cast<float>(chunk_approx_val) : 0.0f;
                            ...
                            int64_t compute_std =
                                row_active ? static_cast<int64_t>(chunk_std_val) : 0;

                            int64_t compute_approx =
                                row_active ? static_cast<int64_t>(chunk_approx_val) : 0;
                            ...
                            int32_t compute_std =
                                row_active ? static_cast<int32_t>(chunk_std_val) : 0;

                            int32_t compute_approx =
                                row_active ? static_cast<int32_t>(chunk_approx_val) : 0;
...
int sc_main(int argc, char *argv[])
{
    ...
    if (argc >= 3)
    {
        ...
        std::cout << "[TB_DEMO] Approximate Multiplier Type: " << approx_mul_type << ", Approximate Adder Type: " << approx_add_type << std::endl;
    }
    else
    {
        std::cout << "[TB_DEMO] No approximation types provided via command line. Using default (0, 0) for both multiplier and adder." << std::endl;
    }
    ...
    tb.npu_std = &npu_std;
    tb.npu_approx = &npu_approx;
    tb.npu_gated = &npu_gated;
