...
namespace fx1
{
  struct PerfCounters
  {
    ...
    // Component-level active cycle counters
    ...
    uint64_t total_cycles{0}; // all clocked cycles from start to done
    ...
    void report(const std::string &tag = "") const
    {
      ...
      printf("  SA active cycles  : %llu\n", (unsigned long long)sa_cycles);
      printf("  OBP active cycles : %llu\n", (unsigned long long)obp_cycles);
