    ...
    private:
        ...
        void decode_process()
        {
            ...
            switch (state_a)
            {
                ...
                case DMA_READ_WAIT:
                {
                    ...
                    if (m_dma)
                    {
                        dma_busy = m_dma->is_read_active(0) || m_dma->is_read_active(2);
                    }
                    if (!dma_busy)
                    {
            ...
            switch (state_b)
            {
                case IDLE:
                {
                    ...
                    if (!queue_b.empty())
                    {
                        ...
                        else if (inst.opcode == 0x15) // ELEM_WISE
                        {
                            ...
                            {
                                uint32_t sz = inst.len * sizeof(T_ACT);
                                m_dma->start_read(3, inst.a_addr, 3, 0, sz);
                                if (inst.mode == 0)
                                {
                                    m_dma->start_read(3, inst.b_addr, 3, 0, sz);
                                }
                            }
                            state_b = DMA_READ_WAIT;
                        }
                ...
                case DMA_READ_WAIT:
                {
                    ...
                    if (m_dma)
                    {
                        dma_busy = m_dma->is_read_active(1) || m_dma->is_read_active(3);
                    }
                    if (!dma_busy)
                    {
                        SauriaRichInstruction inst = queue_b.front();
        ...
        void emulate_elem_wise(const SauriaRichInstruction& inst, int act_bank = 2, int b_bank = 3, int out_bank = 4)
        {
            ...
            if (inst.mode == 0) // ADD
            {
                ...
                {
                    T_ACT val_a, val_b;
                    m_sram->read_bank_data(act_bank, i * sizeof(T_ACT), reinterpret_cast<uint8_t*>(&val_a), sizeof(T_ACT));
                    m_sram->read_bank_data(b_bank, i * sizeof(T_ACT), reinterpret_cast<uint8_t*>(&val_b), sizeof(T_ACT));
