...
int sc_main(int argc, char* argv[])
{
    const std::string config = get_opt(argc, argv, "-c",
                                       "platforms/tests/i2c_platform/configs/default.yaml");
    ...
    cdc::platforms::tests::i2c_platform::i2c_platform_top top(
        "i2c_platform", config);
    ...
    const std::uint64_t sim_ms =
        std::stoull(get_opt(argc, argv, "--sim-ms", "5"));

    sc_core::sc_start(
        sc_core::sc_time(static_cast<double>(sim_ms), sc_core::SC_MS));
