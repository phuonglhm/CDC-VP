...
## 0. This document is a v1.4 snapshot. Read this section first.

> **Everything below describes the component as it stood on 2026-08-07, at
> baseline v1.4.** It is kept because it is the only consolidated record of the
> project intent, the frozen source and the reasoning behind the early
> decisions — not because it is current. Do **not** take sections 2 to 13 as a
> description of the tree you are working on.
>
> Changed since this snapshot, and not reflected anywhere below:
>
> | | This document (v1.4) | Current |
> |---|---|---|
> | Component tests | 41/41 | 42/42 |
> | Mutation controls | 51/51 | 60/60 |
> | Signed baseline | v1.4 | v1.5 (2026-08-24), and the tree has since moved past it — see `STATUS.md` |
> | Decisions | through the Step 1–10 history | plus **D1** owner-aware local bypass, **D24**–**D26**, **R-P9-1**/**R-P9-2**/**R-P9-3** |
> | Completion order | not modelled | per-port, per-channel ticket retirement with a contiguous prefix (`retire_ticket`, `prefix_overflows`) |
> | Admission slots | released on the mesh side | owned by the caller until `b_transport` returns (D26) |
>
> **The risk this section exists to prevent is concrete.** Several invariants
> added since are not derivable from anything below, so an engineer following
> the old instruction to "read this first" and then editing
> `src/noc_interconnect.cpp` could undo them without noticing. For the current
> state, read in this order: `STATUS.md`, then
> `docs/signoff/v1.5/SIGNOFF.md`, then
> `../../TPU_V3/docs/TPU_V3_PHASE9_AUDIT.md` §4h–§4l.
>
> Kept rather than deleted, and kept rather than rewritten: deleting it removed
> an architecture source artifact still referenced as required evidence, and
> rewriting 180 KB to match a tree this document was never verified against
> would replace a dated record with an unverified one. It is labelled instead.
...
Status snapshot date: **2026-08-07** — see section 0 for what has changed.
