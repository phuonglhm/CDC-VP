...
| `axi_chimney.hpp` | `hw/floo_axi_chimney.sv` | Timed chimney, **all four quadrants signed**. **Manager request path** — reorder-buffer gates, `aw_w_sel_q` FSM, request arbiter, bypassed output cut: 141 cycles. **Subordinate side** — unpacker, metadata FIFOs, the unconditional `i_aw_out_queue` spill register, response arbiter: 221 cycles. **Manager-side response unpacker** (`axi_chimney_manager_response`) — channel decode, the per-channel `floo_rsp_o.ready` select, and the `RLAST`-qualified counter release: 78 cycles, Step A-1. A-2 composes these blocks per node and model-tests their wiring; there is no composed RTL harness |
...
| `axi_noc.hpp` | `hw/floo_axi_router.sv` (two `floo_router` instances), `hw/floo_axi_chimney.sv` | `axi_mesh_noc` is the raw `req`/`rsp` mesh pair. `axi_noc` adds one complete timed `axi_chimney_node` at every coordinate (A-2) |
...
| `axi_endpoint.hpp` | **no RTL counterpart**: composes the blocks above | Legacy transaction-level AXI manager/subordinate used by isolated reference tests. **Model-side abstraction, not RTL-signable, and no longer in the integrated datapath after A-3** |
| `noc_interconnect.h` / `src/noc_interconnect.cpp` | **no RTL counterpart**: a CDC-VP integration layer | TLM-2.0 wrapper with `bus_router`'s interface plus mesh placement, TLM-payload-to-AXI mapping, and cycle-by-cycle manager/subordinate adapters driving the timed `axi_noc` since A-3. The adapters are **not RTL-derived** |
