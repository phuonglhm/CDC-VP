...
build_root="${BUILD_ROOT:-/tmp/floo_noc_chimney_rsp_crosscheck}"
systemc_build="$build_root/systemc"
verilator_build="$build_root/verilator"
stimulus="$model_root/tests/data/chimney_rsp_stimulus.csv"
model_trace="$build_root/chimney_rsp_model.csv"
rtl_trace="$build_root/chimney_rsp_rtl.csv"
...
cmake --build "$systemc_build" --target chimney_rsp_trace_sc --parallel
"$systemc_build/tests/chimney_rsp_trace_sc" "$stimulus" "$model_trace"
...
verilator --binary --timing -Wno-fatal \
  ...
  -Wno-WIDTHTRUNC \
  -Wno-UNSIGNED \
  --top-module tb_floo_axi_chimney_rsp_trace \
  --Mdir "$verilator_build" \
  -I"$floo_rtl_root/hw/include" \
  -f "$filelist" \
  "$script_dir/axi_chimney/tb_floo_axi_chimney_rsp_trace.sv"

run_log="$build_root/chimney_rsp_rtl_run.log"
"$verilator_build/Vtb_floo_axi_chimney_rsp_trace" \
  "+STIM_FILE=$stimulus" \
...
python3 "$script_dir/compare_traces.py" \
  "$model_trace" "$rtl_trace" "chimney-response" "flits"
