...
def main() -> int:
    ...
    # Most harnesses compare per cycle; a few compare per configuration.
    unit = sys.argv[4] if len(sys.argv) == 5 else "cycles"
    systemc_lines = normalized_lines(systemc_path)
    rtl_lines = normalized_lines(rtl_path)

    if systemc_lines == rtl_lines:
        print(
            f"{label} cross-check PASS: {len(systemc_lines) - 1} {unit} match"
        )
        return 0
