...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-i" || arg == "--input") result.input = next();
            ...
            result.dma_burst = std::stoul(next());
            ...
            const auto value = std::stoull(next());
            if (value == 0 || value > std::numeric_limits<std::uint32_t>::max()) {
                throw std::invalid_argument("--dst-capacity must be 1..2^32-1");
            }
            result.dst_capacity = static_cast<std::uint32_t>(value);
        } else if (arg == "--expect-error") {
            result.expected_error = std::stoul(next());
        } else if (arg == "--mode") {
    ...
    if (result.input.empty() || result.width == 0 || result.height == 0) {
...
class TlmMemory final : public sc_core::sc_module {
public:
    ...
    bool write_direct(std::uint64_t address,
    ...
    bool read_direct(std::uint64_t address,
                     std::span<std::uint8_t> destination) const {
        if (address > bytes_.size() ||
            destination.size() > bytes_.size() - address) {
            return false;
        }
...
private:
    ...
    void write64(std::uint32_t low, std::uint32_t high, std::uint64_t value) {
        write32(low, static_cast<std::uint32_t>(value));
        write32(high, static_cast<std::uint32_t>(value >> 32));
    }

    void run() {
        try {
            if (!memory_.write_direct(src_, raw_)) {
            ...
            if (options_.expected_error.has_value()) {
                const auto actual_error = read32(ERROR_CODE);
                if ((status & STATUS_ERROR) == 0 ||
                    actual_error != *options_.expected_error) {
                    throw std::runtime_error(
                        "expected VPU error " +
                        std::to_string(*options_.expected_error) +
                        ", got status=" + std::to_string(status) +
                        ", error=" + std::to_string(actual_error));
                }
                std::cout << "SystemC-native expected error observed: code="
                          << actual_error << '\n';
            ...
            if (!memory_.read_direct(dst_, bitstream)) {
            ...
            const auto cycles =
                ...
                read32(CYCLES_LO);
            const auto stalls =
                static_cast<std::uint64_t>(read32(STALL_INPUT_FULL)) +
                read32(STALL_PREDICTION_FULL) + read32(STALL_TRANSFORM_FULL) +
                read32(STALL_CABAC_FULL);
            if (options_.require_stall && stalls == 0) {
                throw std::runtime_error(
                    "expected FIFO backpressure but no stall was observed");
            }
            std::cout << "SystemC-native encode complete: frames="
                      << read32(FRAMES_DONE) << ", bytes=" << output_size
                      << ", cycles=" << cycles << ", fifo_stalls=" << stalls
                      << ", fifo_peak=0x" << std::hex
                      << read32(FIFO_MAX_OCCUPANCY) << std::dec
                      << ", sim_time=" << sc_core::sc_time_stamp() << '\n'
                      << "SystemC active: dma_read=" << read32(DMA_READ_ACTIVE)
                      << ", prediction=" << read32(PREDICTION_ACTIVE)
...
int sc_main(int argc, char** argv) {
    try {
        ...
        if (options.frames == 0 || raw.size() < expected) {
            throw std::runtime_error(
                "raw input is truncated or contains no full frame");
        }
        raw.resize(expected);
        ...
        if (capacity64 > std::numeric_limits<std::uint32_t>::max()) {
            throw std::runtime_error(
                "testbench output buffer exceeds 32-bit capacity");
        }
        const auto capacity = options.dst_capacity.value_or(
