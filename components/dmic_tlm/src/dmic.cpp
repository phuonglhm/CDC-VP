...
DmicTLM::DmicTLM(sc_module_name name)
    : sc_module(name)
    , pdm_target_socket("pdm_target_socket")
    ...
    , counter(0)
    , ctrl_reg(0x0000)
    , // Default: Decimation=64 (0x40 << 8), Disabled (bit 0=0). b_transport() already has safeguard
      // to reset decimation to 64.
    fifo_wm_reg(16)
    , // Default watermark
    overrun_flag(false)
...
void DmicTLM::reset() {
   ...
   overrun_flag = false;
   watermark_flag = false;
   while (!pcm_fifo.empty()) {
      pcm_fifo.pop();
   }
   irq_out.write(false);
}

void DmicTLM::handle_reset() {
   if (!reset_n.read()) {
      reset();
   }
}

void DmicTLM::start_of_simulation() {
   if (!reset_n.read()) {
      reset();
   }
}
