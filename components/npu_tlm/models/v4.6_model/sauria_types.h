...
namespace sauria
{
    ...
    // Default System Parameters (Fixed baseline 64x64 INT8/INT8/INT32 hardware spec)
    const int X = 64;      // Systolic array columns
    const int Y = 64;      // Systolic array rows
    ...
    // Arithmetic Types (Baseline Fixed INT8 / INT8 / INT32)
    typedef int8_t  act_t;  // Activation element data type (INT8)
    typedef int8_t  wei_t;  // Weight element data type (INT8)
    typedef int32_t psum_t; // Partial Sum (accumulator) data type (INT32)
    ...
    // Activation Vector
    template <int Y_DIM, typename T_ACT = int8_t>
    struct act_vector_t
    {
