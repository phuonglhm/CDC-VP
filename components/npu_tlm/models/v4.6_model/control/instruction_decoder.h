...
namespace sauria
{
    struct SauriaRichInstruction
    {
        ...
        uint32_t a_len{0};
        uint32_t b_len{0};
        uint32_t mode{0}; // 0=ADD, 1=MAX_POOL, 2=MUL, 3=SUB, 4=DIV
        float scale_a{1.0f};
        float scale_b{1.0f};
    ...
    template <
        ...
        int SRAMA_CAP = 5056, // 158 KB
        int SRAMB_CAP = 5184, // 162 KB
        int SRAMC_CAP = 1536> // 192 KB
    ...
    private:
        ...
        void host_write_process()
        {
            ...
            if (i_host_wren.read())
            {
                ...
                else if (addr == 0x40000464) { r_a_len = data; }
                else if (addr == 0x40000468) { r_b_len = data; }
                ...
                else if (addr == 0x40000310) // Push rich instruction to Lane A
                {
                    ...
                    inst.a_len = (r_a_len > 0) ? r_a_len : r_len;
                    inst.b_len = (r_b_len > 0) ? r_b_len : ((r_dim > 0 && r_dim < r_len) ? r_dim : r_len);
        ...
        void decode_process()
        {
            ...
            switch (state_a)
            {
                case IDLE:
                {
                    ...
                    if (!queue_a.empty())
                    {
                        ...
                        else if (inst.opcode == 0x12) // GEMM_FUSED
                        {
                            ...
                            if (m_dma)
                            {
                                // CH0: Weights prefetch (Bank 0 max 81 KB per ping-pong buffer)
                                uint32_t wei_size = std::min(static_cast<uint32_t>(inst.k * inst.n * sizeof(T_WEI)), static_cast<uint32_t>(81 * 1024));
                                m_dma->start_read(0, inst.w_addr, 0, 0, wei_size);

                                // CH2: IFmap prefetch (Bank 2 max 79 KB per ping-pong buffer)
                                uint32_t act_size = std::min(static_cast<uint32_t>(inst.m * inst.k * sizeof(T_ACT)), static_cast<uint32_t>(79 * 1024));
                                m_dma->start_read(2, inst.in_addr, 2, 0, act_size);

                                // Optional Skip Connection prefetch (Bank 3 max 79 KB)
                                if (inst.has_skip)
                                {
                                    uint32_t skip_size = std::min(static_cast<uint32_t>(inst.m * inst.n * sizeof(T_ACT)), static_cast<uint32_t>(79 * 1024));
                        ...
                        else if (inst.opcode == 0x15) // ELEM_WISE
                        {
                            ...
                            if (m_dma)
                            {
                                uint32_t sz_a = (inst.a_len > 0 ? inst.a_len : inst.len) * sizeof(T_ACT);
                                m_dma->start_read(2, inst.a_addr, 2, 0, sz_a); // A to Bank 2
                                if (inst.mode != 1) // Modes 0, 2, 3, 4 need B
                                {
                                    uint32_t sz_b = (inst.b_len > 0 ? inst.b_len : (inst.dim > 0 && inst.dim < inst.len ? inst.dim : inst.len)) * sizeof(T_ACT);
                ...
                case DMA_READ_WAIT:
                {
                    ...
                    if (m_dma)
                    {
                        dma_busy = m_dma->is_read_active(0) || m_dma->is_read_active(1) || m_dma->is_read_active(2) || m_dma->is_read_active(3) || m_dma->is_write_active();
                    }
                    if (!dma_busy)
                    {
            ...
            switch (state_b)
            {
                case IDLE:
                {
                    ...
                    if (!queue_b.empty())
                    {
                        ...
                        else if (inst.opcode == 0x12) // GEMM_FUSED
                        {
                            ...
                            {
                                uint32_t wei_size = std::min(static_cast<uint32_t>(inst.k * inst.n * sizeof(T_WEI)), static_cast<uint32_t>(81 * 1024));
                                m_dma->start_read(1, inst.w_addr, 1, 0, wei_size); // CH1, Bank 1 (81 KB)

                                uint32_t act_size = std::min(static_cast<uint32_t>(inst.m * inst.k * sizeof(T_ACT)), static_cast<uint32_t>(79 * 1024));
                                m_dma->start_read(3, inst.in_addr, 3, 0, act_size); // CH3, Bank 3 (79 KB)
                        ...
                        else if (inst.opcode == 0x14) // LAYERNORM
                        {
                            ...
                            if (m_dma)
                            {
                                uint32_t scale_sz = inst.dim * sizeof(T_WEI);
                                uint32_t in_sz = inst.seq_len * inst.dim * sizeof(T_ACT);
                                m_dma->start_read(0, inst.gamma_addr, 1, 0, scale_sz); // Gamma to Bank 1
                                m_dma->start_read(1, inst.beta_addr, 0, 0, scale_sz);  // Beta to Bank 0
                                m_dma->start_read(2, inst.in_addr, 3, 0, in_sz);       // In to Bank 3
                            }
                            state_b = DMA_READ_WAIT;
                        }
                        else if (inst.opcode == 0x15) // ELEM_WISE
                ...
                case COMPUTE_WAIT:
                {
                    ...
                    if (compute_cycles_left_b == 0)
                    {
                        ...
                        {
                            emulate_layernorm(inst, 3, 1, 0, 5);
                            if (m_dma)
                            {
                                uint32_t out_sz = inst.seq_len * inst.dim * sizeof(T_ACT);
                                m_dma->start_write(inst.out_addr, 5, 0, out_sz);
            ...
            for (uint32_t r = 0; r < inst.m; r++)
            {
                for (uint32_t c = 0; c < inst.n; c++)
                {
                    ...
                    for (uint32_t i = 0; i < inst.k; i++)
                    {
                        ...
                        if constexpr (std::is_integral_v<T_ACT> && std::is_integral_v<T_WEI>)
                        {
                            int_sum += static_cast<int64_t>(static_cast<int8_t>(act_val)) * static_cast<int64_t>(static_cast<int8_t>(wei_val));
                        }
                        else
                        {
                            float_sum += static_cast<double>(act_val) * static_cast<double>(wei_val);
                        }
                    }
                    ...
                    double total_scale = scale_in * scale_w * scale_out;
                    ...
                    if constexpr (std::is_integral_v<T_ACT> && std::is_integral_v<T_WEI>)
                    {
                        if (inst.bias_addr != 0 && m_dram)
                        {
                            ...
                            if (b_addr + sizeof(T_PSUM) <= m_dram->size())
                            {
                                ...
                                int_sum += static_cast<int64_t>(b_val);
                            }
                        }
                        val = (total_scale != 1.0) ? (static_cast<double>(int_sum) * total_scale) : static_cast<double>(int_sum);
                    }
                    else
                    {
                        float_sum = float_sum * scale_in * scale_w;
                        if (inst.bias_addr != 0 && m_dram)
                        {
                            uint32_t b_addr = inst.bias_addr + c * sizeof(T_PSUM);
                            if (b_addr + sizeof(T_PSUM) <= m_dram->size())
                            {
                                T_PSUM b_val = *reinterpret_cast<T_PSUM*>(&(*m_dram)[b_addr]);
                                float_sum += static_cast<double>(b_val);
                            }
                        }
                        val = float_sum * scale_out;
                    }
                    ...
                    {
                        double sigmoid = 1.0 / (1.0 + std::exp(-val));
                        val = val * sigmoid;
                    }
                    ...
                    if (inst.has_skip)
                    {
                        ...
                        val += static_cast<double>(skip_val);
                    }
                    
                    T_ACT out_val;
                    if constexpr (std::is_integral_v<T_ACT>)
                    {
                        out_val = static_cast<T_ACT>(std::clamp<int32_t>(static_cast<int32_t>(std::round(val)), -128, 127));
                    }
                    else
                    {
                        out_val = static_cast<T_ACT>(val);
                    }
        ...
        void emulate_layernorm(const SauriaRichInstruction& inst, int act_bank = 2, int gamma_bank = 0, int beta_bank = 1, int out_bank = 4)
        {
            if (!m_sram) return;
            ...
            for (uint32_t i = 0; i < inst.seq_len; i++)
            {
                ...
                for (uint32_t d = 0; d < inst.dim; d++)
                {
                    ...
                    m_sram->read_bank_data(gamma_bank, d * sizeof(T_WEI), reinterpret_cast<uint8_t*>(&gamma_val), sizeof(T_WEI));
                    m_sram->read_bank_data(beta_bank, d * sizeof(T_WEI), reinterpret_cast<uint8_t*>(&beta_val), sizeof(T_WEI));
        ...
        void emulate_elem_wise(const SauriaRichInstruction& inst, int act_bank = 2, int b_bank = 3, int out_bank = 4)
        {
            ...
            uint32_t a_len = (inst.a_len > 0) ? inst.a_len : inst.len;
            uint32_t b_len = (inst.b_len > 0) ? inst.b_len : ((inst.dim > 0 && inst.dim < inst.len) ? inst.dim : inst.len);
            if (a_len == 0) a_len = 1;
            if (b_len == 0) b_len = 1;
            ...
            else if (inst.mode == 2) // MUL
            {
                for (uint32_t i = 0; i < inst.len; i++)
                {
                    ...
                    double res;
                    if (inst.scale_a != 1.0f || inst.scale_b != 1.0f)
                    {
                        res = so * (sa * static_cast<double>(val_a) * sb * static_cast<double>(val_b));
                    }
                    else
                    {
                        res = std::round((static_cast<double>(val_a) * static_cast<double>(val_b)) / 128.0);
                    }
                    T_ACT out_val = clamp_and_cast<T_ACT>(res);
                    m_sram->write_bank_data(out_bank, i * sizeof(T_ACT), reinterpret_cast<const uint8_t*>(&out_val), sizeof(T_ACT));
                }
            ...
            else if (inst.mode == 4) // DIV
            {
                for (uint32_t i = 0; i < inst.len; i++)
                {
                    ...
                    if (db == 0.0) db = 1.0;
                    double res = std::round((sa * static_cast<double>(val_a)) / db);
                    T_ACT out_val = clamp_and_cast<T_ACT>(res);
                    m_sram->write_bank_data(out_bank, i * sizeof(T_ACT), reinterpret_cast<const uint8_t*>(&out_val), sizeof(T_ACT));
                }
            }
