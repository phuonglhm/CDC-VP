...
namespace sauria
{
    ...
    template <
        ...
        int SRAMA_CAP = 5056, // 158 KB (5056 * 32 B)
        int SRAMB_CAP = 5184, // 162 KB (5184 * 32 B)
        int SRAMC_CAP = 1536> // 192 KB (1536 * 128 B)
    ...
    private:
        ...
        // Datatype-agnostic capacity constraints based on HAS Section 6.12 specification:
        // Weight SRAM: 162 KB total (Two 81 KB physical buffers)
        // IFmap SRAM:  158 KB total (Two 79 KB physical buffers)
        // PSum SRAM:   192 KB total (Two 96 KB physical buffers)
        // Scratch SRAM: 48 KB total (Two 24 KB scratchpad buffers)
        static constexpr int ELEMENT_SIZE_A = sizeof(T_ACT);
        static constexpr int VECTOR_SIZE_A = Y_DIM * ELEMENT_SIZE_A;
        static constexpr int SRAMA_CAP_A = (79 * 1024) / (VECTOR_SIZE_A > 0 ? VECTOR_SIZE_A : 1); // 2,528 vectors (79 KB)
        static constexpr int SRAMA_CAP_B = (79 * 1024) / (VECTOR_SIZE_A > 0 ? VECTOR_SIZE_A : 1); // 2,528 vectors (79 KB)

        static constexpr int ELEMENT_SIZE_B = sizeof(T_WEI);
        static constexpr int VECTOR_SIZE_B = X_DIM * ELEMENT_SIZE_B;
        static constexpr int SRAMB_CAP_A = (81 * 1024) / (VECTOR_SIZE_B > 0 ? VECTOR_SIZE_B : 1); // 2,592 vectors (81 KB)
        static constexpr int SRAMB_CAP_B = (81 * 1024) / (VECTOR_SIZE_B > 0 ? VECTOR_SIZE_B : 1); // 2,592 vectors (81 KB)

        static constexpr int ELEMENT_SIZE_C = sizeof(T_PSUM);
        static constexpr int VECTOR_SIZE_C = Y_DIM * ELEMENT_SIZE_C;
        static constexpr int SRAMC_CAP_A = (96 * 1024) / (VECTOR_SIZE_C > 0 ? VECTOR_SIZE_C : 1); // 768 vectors (96 KB)
        static constexpr int SRAMC_CAP_B = (96 * 1024) / (VECTOR_SIZE_C > 0 ? VECTOR_SIZE_C : 1); // 768 vectors (96 KB)

        static constexpr int SCRATCH_SIZE = 24 * 1024; // 24 KB Scratch SRAM per lane

        // Subword offsets for host addressing (datatype-agnostic)
        static constexpr uint32_t SRAMA_REG_B_OFFSET  = (79 * 1024) / (4 * sizeof(T_ACT));  // 20,224 words
        static constexpr uint32_t SRAMA_TOTAL_SIZE    = (158 * 1024) / (4 * sizeof(T_ACT)); // 40,448 words

        static constexpr uint32_t SRAMB_BANK_1_OFFSET = (81 * 1024) / (4 * sizeof(T_WEI));  // 20,736 words
        static constexpr uint32_t SRAMB_TOTAL_SIZE    = (162 * 1024) / (4 * sizeof(T_WEI)); // 41,472 words

        static constexpr uint32_t SRAMC_BANK_4_PSUMS   = (96 * 1024) / (4 * sizeof(T_PSUM)); // 24,576 words
        static constexpr uint32_t SRAMC_BANK_4_SCRATCH = SRAMC_BANK_4_PSUMS + (24 * 1024) / (4 * sizeof(T_PSUM));
        static constexpr uint32_t SRAMC_BANK_5_PSUMS   = SRAMC_BANK_4_SCRATCH + (96 * 1024) / (4 * sizeof(T_PSUM));
        static constexpr uint32_t SRAMC_BANK_5_SCRATCH = SRAMC_BANK_5_PSUMS + (24 * 1024) / (4 * sizeof(T_PSUM));


        // --- 8 physical SRAM Banks ---
        // Banks 0 & 1: Weight SRAM (162 KB total: Two 81 KB ping-pong buffers)
        wei_vector_t<X_DIM, T_WEI> weight_bank_0[SRAMB_CAP_A]; // Bank 0: Lane A weights (81 KB)
        wei_vector_t<X_DIM, T_WEI> weight_bank_1[SRAMB_CAP_B]; // Bank 1: Lane B weights (81 KB)

        // Banks 2 & 3: IFmap SRAM (158 KB total: Two 79 KB ping-pong buffers)
        act_vector_t<Y_DIM, T_ACT> ifmap_bank_0[SRAMA_CAP_A];  // Bank 2: Region A activations (79 KB)
        act_vector_t<Y_DIM, T_ACT> ifmap_bank_1[SRAMA_CAP_B];  // Bank 3: Region B activations (79 KB)

        // Bank 4: PSums_A (96 KB) + Scratch_A (24 KB)
        psum_vector_t<Y_DIM, T_PSUM> psums_bank_4[SRAMC_CAP_A]; // Lane A PSums (96 KB / 768 vectors)
        uint8_t scratch_bank_4[SCRATCH_SIZE];                  // Lane A Scratch (24 KB)

        // Bank 5: PSums_B (96 KB) + Scratch_B (24 KB)
        psum_vector_t<Y_DIM, T_PSUM> psums_bank_5[SRAMC_CAP_B]; // Lane B PSums (96 KB / 768 vectors)
