# SAURIA NPU — Hardware Feedback & Architecture Specification Document

> **Document Name**: `feedback_document.md`  
> **Target System**: SAURIA NPU Core SystemC Cycle-Accurate Model  
> **Date**: August 16, 2026
...
## 2. Comprehensive Register Map & Audit

Below is the detailed specification and hardware audit for the control, queue, interrupt, activation LUT, and DMA performance registers in the current model:
...
| **`FX1_QUEUE_A_STATUS`** | N/A | *Internal C++ Queue* | Queue depth/fullness is managed in `instruction_decoder.h` (`std::queue<SauriaRichInstruction> queue_a`). Not exposed as a host MMIO readback CSR. |
| **`FX1_QUEUE_B_STATUS`** | N/A | *Internal C++ Queue* | Queue depth/fullness is managed in `instruction_decoder.h` (`std::queue<SauriaRichInstruction> queue_b`). Not exposed as a host MMIO readback CSR. |
...
| **`FX1_LUT_B_BASE`** | `0x40160000` | **EXISTS** | Host R/W MMIO base address for **Lane B OBP Activation LUT** (Local Offset `0x00160000`, 16 KB SRAM, $64 \times 256$ entries). |
| **`FX1_DMA_BYTES_READ`**| N/A | *Perf Counter Field* | Measured dynamically by the 60-metric instrumentation engine (`fx1::PerfCounters::ddr_read_bytes` in `perf_counters.h`). Reported in simulation performance logs. |
| **`FX1_DMA_BYTES_WRITTEN`**| N/A | *Perf Counter Field* | Measured dynamically by the 60-metric instrumentation engine (`fx1::PerfCounters::ddr_write_bytes` in `perf_counters.h`). Reported in simulation performance logs. |
...
The **SAURIA AXI DMA Engine** (`sauria_dma.h`) provides high-speed, 4-channel burst data transfers between system DRAM and local SRAM scratchpad banks.
...
The **Output Post-Processing Block (OBP)** in `obp_top.h` implements a 4-stage post-processing epilogue pipeline. **Stage 3** houses a 64-lane parallel SRAM Activation LUT.
...
* **Lane B LUT Base Address (`FX1_LUT_B_BASE`)**: `0x40160000` (Local MMIO Offset: `0x00160000`).
* **Capacity & Structure**: **16 KB (16,384 bytes)** per lane.
  * $64 \text{ parallel PE lanes} \times 256 \text{ entries/lane} \times 1 \text{ byte/entry}$.
  * Declared in `obp_top.h` as `uint8_t lut_ram[Y_DIM][256];` (where `Y_DIM = 64`).
...
```cpp
// Implementation in OBP activation stage:
...
Rich instructions are formatted as 64-bit words submitted via MMIO configuration registers into `instruction_decoder.h`.
...
Mapped at MMIO Address **`0x40000454`** (Offset `0x00454`) in `instruction_decoder.h`.
...
```cpp
// Decoding logic in instruction_decoder.h:
...
Defined in `re_rce.h`. The RCE manages non-linear lookups (`exp`, `recip`, `rsqrt`) while the RE coordinates reduction operations.
...
## 9. Model Verification Summary

### 9.1. Pass Rate Breakdown
* **`yolov8m-int8.onnx`**: **261 / 261 Checkpoints PASS (100.0%)** across all 1,084 nodes and 23 layers ($\text{MAE} = 0.0000$, $\text{Cos Sim} = 1.000000$).
* **`vit_b-int8.onnx`**: **497 / 497 Checkpoints PASS (100.0%)** across all 2,297 nodes and 12 Transformer blocks ($\text{MAE} = 0.0000$, $\text{Cos Sim} = 1.000000$).
* **`sample_vit_block.onnx`**: **4 / 4 Checkpoints PASS (100.0%)** ($\text{MAE} = 0.0000$, $\text{Cos Sim} = 1.000000$).

### 9.2. Recent Hardware & Model Fixes Applied
1. **Dual-Queue DMA Bank Assignment**: Corrected Queue B DMA read destination (`m_dma->start_read(2, ...)` for Operand A and `m_dma->start_read(3, ...)` for Operand B) in `instruction_decoder.h`.
...
4. **PWL Interpolation**: Added PWL linear interpolation and mathematical fallbacks in `re_rce.h`.
5. **Pure INT8 GEMM Emulation**: Implemented pure signed 8-bit integer multiplication and 32-bit accumulation with $[-128, 127]$ saturation in `instruction_decoder.h`.

---

> **Author**: SAURIA NPU Architecture Team  
