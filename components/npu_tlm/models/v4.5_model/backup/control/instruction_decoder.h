                ...
                case IDLE:
                {
                    ...
                    if (!queue_a.empty())
                    {
                        ...
                        else if (inst.opcode == 0x12) // GEMM_FUSED
                        {
                            ...
                            if (m_dma)
                            {
                                // CH0: Weights prefetch (Bank 0 max 320 KB)
                                uint32_t wei_size = std::min(static_cast<uint32_t>(inst.k * inst.n * sizeof(T_WEI)), static_cast<uint32_t>(320 * 1024));
                                m_dma->start_read(0, inst.w_addr, 0, 0, wei_size);

                                // CH2: IFmap prefetch (Bank 2 max 416 KB)
                                uint32_t act_size = std::min(static_cast<uint32_t>(inst.m * inst.k * sizeof(T_ACT)), static_cast<uint32_t>(416 * 1024));
                                m_dma->start_read(2, inst.in_addr, 2, 0, act_size);

                                // Optional Skip Connection prefetch (Bank 3 max 408 KB)
                                if (inst.has_skip)
                                {
                                    uint32_t skip_size = std::min(static_cast<uint32_t>(inst.m * inst.n * sizeof(T_ACT)), static_cast<uint32_t>(408 * 1024));
                    ...
                    if (!queue_b.empty())
                    {
                        ...
                        else if (inst.opcode == 0x12) // GEMM_FUSED
                        {
                            ...
                            if (m_dma)
                            {
                                ...
                                uint32_t act_size = std::min(static_cast<uint32_t>(inst.m * inst.k * sizeof(T_ACT)), static_cast<uint32_t>(408 * 1024));
