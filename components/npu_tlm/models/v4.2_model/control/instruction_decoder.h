...
namespace sauria
{
    ...
    private:
        ...
        uint32_t r_nsplit{16}; // Default split
    ...
    private:
        ...
        void host_write_process()
        {
            ...
            if (i_host_wren.read())
            {
                ...
                else if (addr == 0x40000454) { r_num_heads = data; r_dim = data; r_mode = data; }
                ...
                else if (addr == 0x40000304)
                {
                    ...
                    rich_inst.opcode = (instr >> 56) & 0xFF;
        ...
        void decode_process()
        {
            ...
            switch (state_a)
            {
                case IDLE:
                {
                    ...
                    if (!queue_a.empty())
                    {
                        ...
                        else if (inst.opcode == 0x13) // FUSED_ATTN
                        {
                            ...
                            if (m_dma)
                            {
                                uint32_t sz = inst.seq_len * inst.head_dim * sizeof(T_ACT);
                        ...
                        else if (inst.opcode == 0x15) // ELEM_WISE
                        {
                            ...
                            if (m_dma)
                            {
                                uint32_t sz = inst.len * sizeof(T_ACT);
                                m_dma->start_read(2, inst.a_addr, 2, 0, sz); // A to Bank 2
                                if (inst.mode == 0) // ADD needs B
                                {
                                    m_dma->start_read(3, inst.b_addr, 3, 0, sz); // B to Bank 3
                                }
                            }
                            state_a = DMA_READ_WAIT;
                        }
                ...
                case DMA_READ_WAIT:
                {
                    ...
                    if (m_dma)
                    {
                        dma_busy = m_dma->is_any_read_active();
                    }
                    if (!dma_busy)
                    {
                ...
                case COMPUTE_WAIT:
                {
                    ...
                    if (compute_cycles_left_a == 0)
                    {
                        ...
                        else if (inst.opcode == 0x13) // FUSED_ATTN
                        {
                            ...
                            if (m_dma)
                            {
                                uint32_t out_sz = inst.seq_len * inst.head_dim * sizeof(T_ACT);
            ...
            // --------------------------------------------------------
            // --- Lane B Decoder & Executor (Mirroring compute only) ---
            // --------------------------------------------------------
            if (!queue_b.empty() && state_b == IDLE)
            {
        ...
        void emulate_gemm_fused(const SauriaRichInstruction& inst)
        {
            if (!m_sram) return;
            ...
            for (uint32_t r = 0; r < inst.m; r++)
            {
                for (uint32_t c = 0; c < inst.n; c++)
                {
                    ...
                    {
                        T_ACT act_val;
                        m_sram->read_bank_data(2, (r * inst.k + i) * sizeof(T_ACT), reinterpret_cast<uint8_t*>(&act_val), sizeof(T_ACT));
                        
                        T_WEI wei_val;
                        m_sram->read_bank_data(0, (i * inst.n + c) * sizeof(T_WEI), reinterpret_cast<uint8_t*>(&wei_val), sizeof(T_WEI));
                        
                        sum += static_cast<double>(act_val) * static_cast<double>(wei_val);
                    }
                    
                    sum = sum * inst.in_scale * inst.w_scale;
                    ...
                    sum = sum * inst.out_scale;
                    
                    T_ACT out_val = static_cast<T_ACT>(sum);
                    m_sram->write_bank_data(4, (r * inst.n + c) * sizeof(T_ACT), reinterpret_cast<const uint8_t*>(&out_val), sizeof(T_ACT));
                }
            }
        }

        void emulate_fused_attn(const SauriaRichInstruction& inst)
        {
            if (!m_sram) return;
                ...
                for (uint32_t i = 0; i < inst.seq_len; i++)
                {
                    for (uint32_t j = 0; j < inst.seq_len; j++)
                    {
                        ...
                        for (uint32_t d = 0; d < inst.head_dim; d++)
                        {
                            ...
                            m_sram->read_bank_data(0, q_offset, reinterpret_cast<uint8_t*>(&q_val), sizeof(T_ACT));
                            m_sram->read_bank_data(1, k_offset, reinterpret_cast<uint8_t*>(&k_val), sizeof(T_ACT));
                            sum += static_cast<double>(q_val) * static_cast<double>(k_val);
                        }
                        qk[i * inst.seq_len + j] = sum * inst.attn_scale;
                    }
                }
                ...
                for (uint32_t i = 0; i < inst.seq_len; i++)
                {
                    for (uint32_t d = 0; d < inst.head_dim; d++)
                    {
                        ...
                        for (uint32_t j = 0; j < inst.seq_len; j++)
                        {
                            ...
                            m_sram->read_bank_data(2, v_offset, reinterpret_cast<uint8_t*>(&v_val), sizeof(T_ACT));
                            sum += attn_val * static_cast<double>(v_val);
                        }
                        T_ACT out_val = static_cast<T_ACT>(sum);
                        uint32_t out_offset = (i * num_heads * inst.head_dim + h * inst.head_dim + d) * sizeof(T_ACT);
                        m_sram->write_bank_data(4, out_offset, reinterpret_cast<const uint8_t*>(&out_val), sizeof(T_ACT));
                    }
                }
            }
        }

        void emulate_layernorm(const SauriaRichInstruction& inst)
        {
            if (!m_sram) return;
            ...
            for (uint32_t i = 0; i < inst.seq_len; i++)
            {
                ...
                {
                    T_ACT val;
                    m_sram->read_bank_data(2, (i * inst.dim + d) * sizeof(T_ACT), reinterpret_cast<uint8_t*>(&val), sizeof(T_ACT));
                ...
                for (uint32_t d = 0; d < inst.dim; d++)
                {
                    ...
                    m_sram->read_bank_data(0, d * sizeof(T_WEI), reinterpret_cast<uint8_t*>(&gamma_val), sizeof(T_WEI));
                    m_sram->read_bank_data(1, d * sizeof(T_WEI), reinterpret_cast<uint8_t*>(&beta_val), sizeof(T_WEI));
                    ...
                    T_ACT out_val = static_cast<T_ACT>(scaled_val);
                    m_sram->write_bank_data(4, (i * inst.dim + d) * sizeof(T_ACT), reinterpret_cast<const uint8_t*>(&out_val), sizeof(T_ACT));
                }
            }
        }

        void emulate_elem_wise(const SauriaRichInstruction& inst)
        {
            if (!m_sram) return;
            ...
            if (inst.mode == 0) // ADD
            {
                ...
                {
                    T_ACT val_a, val_b;
                    m_sram->read_bank_data(2, i * sizeof(T_ACT), reinterpret_cast<uint8_t*>(&val_a), sizeof(T_ACT));
                    m_sram->read_bank_data(3, i * sizeof(T_ACT), reinterpret_cast<uint8_t*>(&val_b), sizeof(T_ACT));
                    
                    double res = inst.scale_out * (inst.scale_a * static_cast<double>(val_a) + inst.scale_b * static_cast<double>(val_b));
                    T_ACT out_val = static_cast<T_ACT>(res);
                    m_sram->write_bank_data(4, i * sizeof(T_ACT), reinterpret_cast<const uint8_t*>(&out_val), sizeof(T_ACT));
                }
            }
            else if (inst.mode == 1) // MAX_POOL
            {
                ...
                {
                    T_ACT max_val;
                    m_sram->read_bank_data(2, (i * stride) * sizeof(T_ACT), reinterpret_cast<uint8_t*>(&max_val), sizeof(T_ACT));
                    double max_double = static_cast<double>(max_val);
                    
                    for (uint32_t w = 1; w < stride; w++)
                    {
                        T_ACT val;
                        m_sram->read_bank_data(2, (i * stride + w) * sizeof(T_ACT), reinterpret_cast<uint8_t*>(&val), sizeof(T_ACT));
                        max_double = std::max(max_double, static_cast<double>(val));
                    }
                    T_ACT out_val = static_cast<T_ACT>(max_double);
                    m_sram->write_bank_data(4, i * sizeof(T_ACT), reinterpret_cast<const uint8_t*>(&out_val), sizeof(T_ACT));
                }
            }
        }
