...
namespace sauria
{
    ...
    public:
        ...
        // re-routes to the array, OBP, DMA, RE, and RCE instances.
        ...
        void attach_perf(fx1::PerfCounters *p)
        {
            ...
            if (array_inst) array_inst->perf = p;
            if (obp_inst_a) obp_inst_a->perf = p;
            if (obp_inst_b) obp_inst_b->perf = p;
            if (dma_inst)   dma_inst->perf = p;
            if (re_inst_a)  re_inst_a->perf = p;
            if (re_inst_b)  re_inst_b->perf = p;
            if (rce_inst_a) rce_inst_a->perf = p;
            if (rce_inst_b) rce_inst_b->perf = p;
        }

        void perf_monitor_logic()
        {
            if (i_rstn.read() && perf)
            {
                perf->total_cycles++;
                if (s_act_stall_a.read() || s_act_stall_b.read() || s_wei_stall_a.read() || s_wei_stall_b.read())
                {
                    perf->stall_cycles++;
                }
            }
        ...
        uint32_t get_nsplit() const { return s_nsplit.read(); }
        ...
        NpuTop(sc_module_name nm, const PeConfig &pe_cfg = PeConfig()) : sc_module(nm)
        {
            ...
            sram_inst = new Sram<X_DIM, Y_DIM, T_ACT, T_WEI, T_PSUM>("sram_inst");
            ...
            act_feeder_b = new IfmapFeeder<Y_DIM, T_ACT, 1024, FIFO_DEPTH, true>("act_feeder_b");
            wei_feeder_a = new WeightFeeder<X_DIM, T_WEI, 1024, FIFO_DEPTH, 0, false>("wei_feeder_a");
            wei_feeder_b = new WeightFeeder<X_DIM, T_WEI, 1024, FIFO_DEPTH, 0, true>("wei_feeder_b");
            array_inst = new SystolicArray<X_DIM, Y_DIM, T_ACT, T_WEI, T_PSUM>("array_inst", pe_cfg);
            psm_inst_a = new Psm<X_DIM, Y_DIM, T_PSUM>("psm_inst_a");
            psm_inst_b = new Psm<X_DIM, Y_DIM, T_PSUM>("psm_inst_b");
            ...
            dma_inst = new SauriaDma<X_DIM, Y_DIM, T_ACT, T_WEI, T_PSUM>("dma_inst");
            decoder_inst = new InstructionDecoder<X_DIM, Y_DIM, T_ACT, T_WEI, T_PSUM>("decoder_inst");
            dma_inst->set_sram(sram_inst);
            decoder_inst->set_dma(dma_inst);
            decoder_inst->set_sram(sram_inst);

#ifndef FX1_NO_PERF
            // Forward the externally-attached perf struct down to submodules.
            ...
            if (dma_inst)   dma_inst->perf = perf;
            if (re_inst_a)  re_inst_a->perf = perf;
            if (re_inst_b)  re_inst_b->perf = perf;
            if (rce_inst_a) rce_inst_a->perf = perf;
            if (rce_inst_b) rce_inst_b->perf = perf;

            SC_METHOD(perf_monitor_logic);
            sensitive << i_clk.pos();
            dont_initialize();
#endif
            ...
            SC_METHOD(active_merge_logic);
            sensitive << s_ctrl_active_a << s_ctrl_active_b;
            ...
            config_regs_inst->i_active(s_npu_active);
    ...
    public:
        ...
        Sram<X_DIM, Y_DIM, T_ACT, T_WEI, T_PSUM> *sram_inst{nullptr};
        ...
        IfmapFeeder<Y_DIM, T_ACT, 1024, FIFO_DEPTH, true> *act_feeder_b{nullptr};
        WeightFeeder<X_DIM, T_WEI, 1024, FIFO_DEPTH, 0, false> *wei_feeder_a{nullptr};
        WeightFeeder<X_DIM, T_WEI, 1024, FIFO_DEPTH, 0, true> *wei_feeder_b{nullptr};
        SystolicArray<X_DIM, Y_DIM, T_ACT, T_WEI, T_PSUM> *array_inst{nullptr};
        Psm<X_DIM, Y_DIM, T_PSUM> *psm_inst_a{nullptr};
        Psm<X_DIM, Y_DIM, T_PSUM> *psm_inst_b{nullptr};
        ...
        InstructionDecoder<X_DIM, Y_DIM, T_ACT, T_WEI, T_PSUM> *decoder_inst{nullptr};
        SauriaDma<X_DIM, Y_DIM, T_ACT, T_WEI, T_PSUM> *dma_inst{nullptr};
    ...
    private:
        ...
        // Deadlock & Active outputs from A and B controllers
        sc_signal<bool> s_deadlock_a{"s_deadlock_a"};
        sc_signal<bool> s_deadlock_b{"s_deadlock_b"};
        sc_signal<bool> s_npu_active{"s_npu_active"};
        ...
        void done_latch_logic()
        {
            ...
            else
            {
                ...
                if (nsplit == 0)
                {
                    o_done.write(done_latched_reg_b);
                }
        ...
        void active_merge_logic()
        {
            s_npu_active.write(s_ctrl_active_a.read() || s_ctrl_active_b.read());
        }
