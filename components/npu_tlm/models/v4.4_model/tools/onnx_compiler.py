...
class OnnxCompiler:
    def __init__(self, model_path, output_cpp, dtype='int8', pe_x=64, pe_y=64):
    ...
    def get_tensor_data(self, name, default_shape=[64, 64]):
    ...
    def find_scale(self, tensor_name, default=1.0):
        scale_name_candidates = [
            tensor_name + '_scale',
            tensor_name + '.scale',
            tensor_name.replace('_quantized', '_scale'),
            tensor_name.replace('.weight', '.scale')
        ]
        for c in scale_name_candidates:
            if c in self.tensors and 'data' in self.tensors[c]:
    ...
    def lower_gemm_conv(self, node):
        ...
        in_shape = self.get_shape(inputs[0], [64, 64])
        w_shape = self.get_shape(inputs[1] if len(inputs) > 1 else inputs[0], [64, 64])

        m = int(np.prod(in_shape[:-1])) if len(in_shape) >= 2 else (in_shape[0] if len(in_shape)==1 else 64)
        k = int(in_shape[-1]) if len(in_shape) >= 1 else 64
        n = int(w_shape[0] if (len(w_shape)==2 and w_shape[1]==k) else w_shape[-1]) if len(w_shape) >= 1 else 64
        ...
        A = pad_or_truncate_2d(self.get_tensor_data(inputs[0], default_shape=[m, k]), (m, k)).astype(np.float32)
        W = pad_or_truncate_2d(self.get_tensor_data(inputs[1] if len(inputs)>1 else inputs[0], default_shape=[k, n]), (k, n)).astype(np.float32)
        ...
        self.tensors[outputs[0]] = {'addr': out_addr, 'shape': [m, n], 'bytes': m*n, 'data': gold_out}
    ...
    def lower_layernorm(self, node):
        ...
        outputs = node.output
        in_shape = self.get_shape(inputs[0], [64, 64])
        seq_len = int(np.prod(in_shape[:-1])) if len(in_shape) >= 2 else (in_shape[0] if len(in_shape)==1 else 64)
        dim = int(in_shape[-1]) if len(in_shape) >= 1 else 64
    ...
    def lower_elemwise(self, node):
        ...
        inputs = node.input
        outputs = node.output
        in_shape_a = self.get_shape(inputs[0], [64, 64])
        in_shape_b = self.get_shape(inputs[1], [64, 64]) if len(inputs) > 1 else in_shape_a

        num_elem_a = int(np.prod(in_shape_a)) if len(in_shape_a) > 0 else 4096
        num_elem_b = int(np.prod(in_shape_b)) if len(in_shape_b) > 0 else num_elem_a
    ...
    def lower_softmax(self, node):
        ...
        in_shape = self.get_shape(inputs[0], [64, 64])
        seq_len = int(np.prod(in_shape[:-1])) if len(in_shape) >= 2 else (in_shape[0] if len(in_shape)==1 else 64)
        head_dim = int(in_shape[-1]) if len(in_shape) >= 1 else 64
        ...
        gold_out = np.clip(np.round(soft), -128, 127).astype(np.int8)
    ...
    def get_shape(self, tensor_name, default_shape=[64, 64]):
    ...
    def get_tensor_addr(self, name, default_shape=[64, 64]):
    ...
    def run_golden_emulation_pass(self):
        ...
        for idx, inst in enumerate(self.instructions):
            ...
            if opcode == 0x12: # GEMM_FUSED
                ...
                A_raw = np.frombuffer(dram_bytes[a_addr:a_addr+length], dtype=np.int8)
                B_raw = np.frombuffer(dram_bytes[b_addr:b_addr+length], dtype=np.int8)
...
"""
        ...
        for idx, inst in enumerate(self.instructions):
            ...
            if inst['opcode'] == 0x12: # GEMM_FUSED
                ...
                head_dim = inst.get('head_dim', inst.get('dim', 64))
                seq_len = inst.get('seq_len', 64)
        ...
        wait_cycles = max(5000, len(self.instructions) * 200)
...
def main():
    ...
    parser.add_argument("--dtype", default="int8", choices=["int8", "fp16", "int16"], help="Target quantization data type")
    parser.add_argument("--eval_x", type=int, default=64, help="PE Array X dimension")
    parser.add_argument("--eval_y", type=int, default=64, help="PE Array Y dimension")
