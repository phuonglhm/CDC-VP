...
namespace sauria
{
    ...
    private:
        ...
        // context_switch_controller state registers
        bool pop_shim_init_q{false};
        bool pop_shim_q2{false};
        uint32_t incnt_q{0};
        bool cdone_hold{false};
        bool cdone_q{false};
        bool cdone_shim_q1{false};
        uint32_t cscnt_q{0};
        sc_bv<X_DIM> cswitch_arr_q{0};
        ...
        void ctrl_process()
        {
            ...
            // Context switch controller update logic:
            bool i_clear = (state == IDLE || state == START_FLAGS);
            
            bool cdone = false;
            bool o_cdone_val = false;
            bool o_cswitch_done_val = false;
            sc_bv<X_DIM> cswitch_arr_d_var(0);
            ...
            {
                // Inputs to the context switch controller
                bool wei_pop = o_wei_pop_en.read();
                bool act_pop = o_act_pop_en.read();
                bool pipeline_en = o_pipeline_en.read();
                bool cswitch_en = (state == START_COMP || state == DRAIN_FEED || state == ARRAY_CSWITCH || state == WAIT_CSWITCH);
                bool cswitch_force = (state == START_FLAGS);
                bool feeders_done = (state != START_COMP && state != ARRAY_PREP && state != ARRAY_FILL && state != START_FLAGS);
                
                // pop_shim_init_d
                bool pop_shim_init_d = pop_shim_init_q;
                if (wei_pop && act_pop) {
                    pop_shim_init_d = true;
                }

                // pop_shim_q2 next value
                bool pop_shim_q2_next = pop_shim_q2;
                if (pipeline_en || cswitch_force) {
                    pop_shim_q2_next = (pop_shim_init_q && wei_pop && act_pop) || feeders_done;
                }

                // cdone_val & forces
                uint32_t cdone_val = (incntlim > 1) ? (incntlim - 2) : 0;
                bool cdone_force_q1 = (incntlim == 1);
                bool cdone_force_q2 = (incntlim == 0);

                cdone = (incnt_q == cdone_val) && (pop_shim_q2 || cdone_force_q2);

                // incnt_d
                uint32_t incnt_d = incnt_q;
                if (pipeline_en && pop_shim_q2) {
                    if (incnt_q == incntlim) {
                        incnt_d = 0;
                    } else {
                        incnt_d = incnt_q + 1;
                ...
                // cdone_hold_d
                bool active_or_draining = (wei_pop && act_pop) || feeders_done;
                bool cdone_hold_d = cdone_hold;
                if (cdone && (pipeline_en || cswitch_force) && active_or_draining) {
                    cdone_hold_d = true;
                } else if (((cswitch_en && cdone_shim_q1) || cswitch_force) && cswitch_en && (cscnt_q == 0) && (pipeline_en || cswitch_force)) {
                    cdone_hold_d = false;
                }

                // cscnt_d
                bool cscnt_trigger = (cswitch_en && cdone_shim_q1) || cswitch_force;
                bool cscnt_flag = false;
                uint32_t cscnt_d = cscnt_q;
                uint32_t cswitch_prop_cycles = PE_LAT + X_DIM + Y_DIM - 1;
                if (cscnt_q == cswitch_prop_cycles) {
                    cscnt_flag = true;
                    cscnt_d = 0;
                } else {
                    if (cscnt_trigger || (cscnt_q > 0)) {
                        cscnt_d = cscnt_q + 1;
                    }
                }
                ...
                for (int i = 0; i < X_DIM; i++) {
                    if (cscnt_q == (uint32_t)(PE_LAT - EXTRA_CSREG + i)) {
                        cswitch_arr_d_var[i] = 1;
                    }
                }

                // Compute output values
                o_cdone_val = (cdone_q || (cdone_force_q1 && cdone)) && pipeline_en && active_or_draining;
                o_cswitch_done_val = cscnt_flag && pipeline_en && cswitch_en;

                // Update registers
                if (pipeline_en || cswitch_force) {
                    pop_shim_init_q = pop_shim_init_d;
                    pop_shim_q2 = pop_shim_q2_next;
                }
                incnt_q = incnt_d;
                if (pipeline_en || cswitch_force) {
                ...
                if (pipeline_en && cswitch_en) {
                    cscnt_q = cscnt_d;
                }
                if (pipeline_en) {
                    cswitch_arr_q = cswitch_arr_d_var;
                }
            }
            if (state != IDLE || (dbg_cycle % 500) == 0)
            {
                DBG_COUT << "[CTRL DEBUG] cycle=" << dbg_cycle
                          ...
                          << " done=" << o_done.read()
                          << " cscnt_q=" << cscnt_q
                          << " pop_shim_q2=" << pop_shim_q2
                          << " cdone_q=" << cdone_q
                          << " o_cswitch_done_val=" << o_cswitch_done_val
                          << " o_cdone_val=" << o_cdone_val
                          << std::endl;
            }
            ...
            o_active.write(state != IDLE && state != DONE);
