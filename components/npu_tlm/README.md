# SAURIA V4.2 CDC-VP Integration

`npu_tlm` connects the SAURIA MP1 V1.1 V4.2 SystemC model to the
...
`0x10200004`, while the CDC compatibility control register at offset
...
RISC-V software
  -> 32-bit MMIO at 0x10200000 + offset
    -> CDC-VP bus_router
      -> npu_tlm::b_transport
        -> native V4.2 host interface, or
        -> CDC compatibility GEMM controller
          -> RAM master socket
...
| Class | Meaning |
| --- | --- |
| Native config RW | V4.2 `config_regs.h` decodes host writes and reads |
| SRAM RW | V4.2 `sram_top.h` decodes host writes and reads |
| Rich WO | V4.2 `instruction_decoder.h` decodes writes; it defines no register readback |
| OBP RW | V4.2 decodes writes and `NpuTop` routes host reads |
| OBP WO | V4.2 decodes writes but `NpuTop` does not route that region on host reads |
| RCE RW | V4.2 decodes writes and `NpuTop` routes host reads |
| Counter RO | CDC-VP returns a raw V4.2 `PerfCounters` field |
| Compatibility RW/RO/W1C | CDC-VP implements the software-facing GEMM controller |

## Aperture Summary

| VP offset/range | Physical address/range | Interface |
| ---: | ---: | --- |
| `0x00000..0x00AFF` | `0x10200000..0x10200AFF` | Native V4.2 control and configuration |
| selected `0x01200..0x01268` | `0x10201200..0x10201268` | Raw V4.2 performance counters |
| `0x10000..0x10FFF` | `0x10210000..0x10210FFF` | Compact rich-instruction alias |
| `0x20000..0x2FFFF` | `0x10220000..0x1022FFFF` | Compact OBP and RCE aliases |
| `0x30000..0x31FFF` | `0x10230000..0x10231FFF` | CDC compatibility GEMM bank |
...
At native control offset `0x0000`, V4.2 implements:
...
| Offset | Access | Firmware symbol | V4.2 field |
| ---: | --- | --- | --- |
...
V4.2 also declares `F_TIL_CKSTEP`, `F_INACTIVE_COLS`, and `F_PRELOAD_EN` at
...
The V4.2 constants `OUT_H`, `OUT_W`, `OUT_C`, and layer `DIL_PAT` exist at
...
base uses V4.2 physical-row and subword encoding; it is not a normal linear
CPU byte array.

## Compact Aliases

Some V4.2 model host addresses are outside the 1 MiB SoC aperture. CDC-VP
translates compact VP offsets to those sparse model addresses.

| VP offset/range | Physical range | Access | V4.2 model range | Region |
| ---: | ---: | --- | ---: | --- |
| selected `0x10300..0x10460` | `0x10210300..0x10210460` | WO | `0x40000300..0x40000460` | Rich instruction decoder |
| `0x20000..0x21FFF` | `0x10220000..0x10221FFF` | RW | `0x00140000..0x00141FFF` | OBP A LUT |
| `0x22000..0x2207F` | `0x10222000..0x1022207F` | RW | `0x00150000..0x0015007F` | OBP A bias |
| `0x23000..0x2307F` | `0x10223000..0x1022307F` | WO | `0x00180000..0x0018007F` | OBP A scale |
| `0x24000..0x2407F` | `0x10224000..0x1022407F` | WO | `0x00190000..0x0019007F` | OBP A shift |
| `0x25000..0x26FFF` | `0x10225000..0x10226FFF` | RW | `0x00160000..0x00161FFF` | OBP B LUT |
| `0x27000..0x2707F` | `0x10227000..0x1022707F` | RW | `0x00170000..0x0017007F` | OBP B bias |
| `0x28000..0x2807F` | `0x10228000..0x1022807F` | WO | `0x001A0000..0x001A007F` | OBP B scale |
| `0x29000..0x2907F` | `0x10229000..0x1022907F` | WO | `0x001B0000..0x001B007F` | OBP B shift |
| `0x2A000..0x2A0FF` | `0x1022A000..0x1022A0FF` | RW | `0x00200000..0x002000FF` | RCE A exponential |
| `0x2B000..0x2B1FF` | `0x1022B000..0x1022B1FF` | RW | `0x00210000..0x002101FF` | RCE A reciprocal |
| `0x2C000..0x2C7FF` | `0x1022C000..0x1022C7FF` | RW | `0x00220000..0x002207FF` | RCE A reciprocal square root |
| `0x2D000..0x2D0FF` | `0x1022D000..0x1022D0FF` | RW | `0x00230000..0x002300FF` | RCE B exponential |
| `0x2E000..0x2E1FF` | `0x1022E000..0x1022E1FF` | RW | `0x00240000..0x002401FF` | RCE B reciprocal |
| `0x2F000..0x2F7FF` | `0x1022F000..0x1022F7FF` | RW | `0x00250000..0x002507FF` | RCE B reciprocal square root |

The compact offsets are CDC-VP integration definitions. The sparse target
addresses and their behavior belong to the V4.2 model.

## Rich Instruction Write Registers

The rich page is write-only because V4.2 does not define host readback for
these instruction-decoder registers.

| VP offset | Physical address | Firmware symbol | V4.2 field or action |
| ---: | ---: | --- | --- |
...
Rich instruction values decoded by V4.2 are:

| Type | Value | V4.2 name |
| --- | ---: | --- |
...
## Raw V4.2 Performance Counters

The wrapper exposes every field that exists in V4.2 `fx1::PerfCounters`.
Each 64-bit counter is a read-only low/high pair.
...
not invent MMIO registers for fields absent from V4.2 `PerfCounters`.

## CDC Compatibility GEMM Bank

The compatibility bank starts at:
...
| `0x0010` | `0x10230010` | `SRC_ADDR` | RW | Physical address of row-major `A[32][K]` |
| `0x0014` | `0x10230014` | `DST_ADDR` | RW | Physical address of row-major `C[32][32]` |
| `0x0018` | `0x10230018` | `SCRATCH_ADDR` | RW | Reserved |
| `0x001C` | `0x1023001C` | `SRC_SIZE_BYTES` | RW | Source span including row stride |
| `0x0020` | `0x10230020` | `DST_SIZE_BYTES` | RW | Must be at least `4096` |
| `0x0024` | `0x10230024` | `WIDTH` | RW | Must be `32` |
| `0x0028` | `0x10230028` | `HEIGHT` | RW | Must be `32` |
...
| `0x0038` | `0x10230038` | `WEIGHTS_ADDR` | RW | Physical address of row-major `B[K][32]` |
| `0x003C` | `0x1023003C` | `PARAM_ADDR` | RW | Reserved |
| `0x0040` | `0x10230040` | `WEIGHTS_SIZE_BYTES` | RW | Must be at least `K * 32` |
| `0x1000` | `0x10231000` | `K_DIMENSION` | RW | Reduction dimension `1..992` |
...
| `0x101C` | `0x1023101C` | `LAST_ERROR` | RO | Compatibility error code |
...
The compatibility operation is:

```text
C[32][32] = A[32][K] x B[K][32]
```

`A` and `B` are signed INT8, `C` is signed INT32, and `K` is `1..992`.
...
| `10` | `CDC_NPU_ERROR_CORE_DEADLOCK` | V4.2 feeder deadlock |
| `11` | `CDC_NPU_ERROR_CORE_TIMEOUT` | V4.2 core timed out |
| `12` | `CDC_NPU_ERROR_RESET_ABORTED` | External reset aborted work |

## Model Files That Are Not MMIO Specifications

The following V4.2 files define driver packing, testbench, target, or stimulus
...
| V4.2 `config_regs.h` and `config_map.h` | Native profile-dependent decode |
| V4.2 `sram/sram_top.h` | Native SRAM host decode |
| V4.2 `control/instruction_decoder.h` | Rich write-register decode |
...
- The compatibility bank is a CDC-VP extension. It is not part of the native
  V4.2 register map.
