...
The tool can be invoked from any directory. Output is written to a canonical location:
- If run from `components/isp_tlm/`: output goes to `components/isp_tlm/output.yuv`
- If run from anywhere else: output goes to `components/isp_tlm/output.yuv`

This keeps the output files in one consistent place.

**Command-line options:**
| Option | Description |
|--------|-------------|
| `-i <path>` | Input RAW file path (required) |
| `-o <path>` | Override output YUV path (default: `components/isp_tlm/output.yuv`) |
...
/home/hoangquan/workspace/CDC-VP/build/bremen/components/isp_tlm/tests/isp_run \
...
cd components/isp_tlm
/home/hoangquan/workspace/CDC-VP/build/bremen/components/isp_tlm/tests/isp_run \
  -i input/A_raw_2688x1520_5376.raw \
...
Successfully saved YUV output to ./output.yuv
Output size: 6128640 bytes
Metadata written to: ./output.yuv.json

========================================
To view the output, run:
  ffplay -f rawvideo -pixel_format yuv420p -video_size 2688x1520 ./output.yuv
========================================
```
...
After running `isp_run`, the following files are written to `components/isp_tlm/`:
...
The script reads dimensions from `output.yuv.json`, displays the input RAW alongside the ISP output, and writes:
- `output.jpg` - Standalone JPEG of the processed output
- `isp_before_after.jpg` - Side-by-side comparison image
...
All ISP output files live in `components/isp_tlm/`:
```
components/isp_tlm/
