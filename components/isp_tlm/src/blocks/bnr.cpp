...
static void joint_bilateral_filter(const std::vector<float>& in_img,
                                   ...
                                   int spatial_size, float stddev_s, float stddev_r, int stride) {
        ...
        for (int c = 0; c < static_cast<int>(w); ++c) {
            ...
            for (int i = 0; i < spatial_size; ++i) {
                ...
                for (int j = 0; j < spatial_size; ++j) {
                    int nc = c + j - half;
                    float guide_neigh = isp_utils::get_pixel_mirror(guide_img.data(), nr, nc, w, h);
                    float in_neigh = isp_utils::get_pixel_mirror(in_img.data(), nr, nc, w, h);
...
void bnr_block::process(const uint16_t* in, uint16_t* out, uint32_t w, uint32_t h, const bnr_config& cfg, cfa_types bayer_pattern, uint8_t bit_depth) {
    if (!cfg.is_enable) {
    ...
    // extract sub-images and populate full interp_g
    ...
    joint_bilateral_filter(in_img_r, interp_g_at_r, out_img_r, sub_w, sub_h, filt_size_r, cfg.r_std_dev_s, cfg.r_std_dev_r, 2);
    joint_bilateral_filter(interp_g, interp_g, out_img_g, w, h, filt_size_g, cfg.g_std_dev_s, cfg.g_std_dev_r, 1);
    joint_bilateral_filter(in_img_b, interp_g_at_b, out_img_b, sub_w, sub_h, filt_size_b, cfg.b_std_dev_s, cfg.b_std_dev_r, 2);
