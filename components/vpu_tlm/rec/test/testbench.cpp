...
bool TestBench::dataflow_test(const CustomPacket &pkt_in) {
    ...
    // Serialize CustomPacket into a contiguous buffer: header then payload (with extended header)
...
bool TestBench::recIntra_DCMode_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
    ...
    // Ensure prediction header fields are explicit for this test and use packCustomPacket
    ...
    CustomPacket out_pkt = unpackCustomPacket(db_monitor.last_data.data(), db_monitor.last_data.size());
...
bool TestBench::recIntra_PlanarMode_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
        pkt.cmd = CustomCmd::READ_REQ;
        pkt.block_idx = 5;
...
bool TestBench::recIntra_AngularMode_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
        pkt.cmd = CustomCmd::READ_REQ;
        pkt.block_idx = 5;
...
bool TestBench::recMc_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
        pkt.cmd = CustomCmd::READ_REQ;
        pkt.block_idx = 5;
    ...
    if (db_monitor.last_data.empty()) return false;
    CustomPacket out_pkt = unpackCustomPacket(db_monitor.last_data.data(), db_monitor.last_data.size());
    if (out_pkt.cmd != CustomCmd::RESIDUAL) return false;
    if (out_pkt.data.size() < 16) return false;
    for (size_t i = 0; i < 16; ++i) {
        if (out_pkt.data[i] != 128) return false;
    }
...
bool TestBench::residual_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
        pkt.cmd = CustomCmd::READ_REQ;
        pkt.block_idx = 5;
...
bool TestBench::tq_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
        pkt.cmd = CustomCmd::READ_REQ;
        pkt.block_idx = 5;
    ...
    CustomPacket out_pkt = unpackCustomPacket(coeff_monitor.last_data.data(), coeff_monitor.last_data.size());
...
bool TestBench::inv_tq_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
        pkt.cmd = CustomCmd::READ_REQ;
        pkt.block_idx = 5;
...
bool TestBench::mcPre_to_db_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
    ...
    if (db_monitor.last_data.empty()) return false;
    CustomPacket out = unpackCustomPacket(db_monitor.last_data.data(), db_monitor.last_data.size());
    if (out.cmd != CustomCmd::RESIDUAL) return false;
...
bool TestBench::mcReadReq_no_mv_fallback_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    if (pkt.data.empty()) {
        pkt.cmd = CustomCmd::READ_REQ;
        pkt.block_idx = 7;
...
bool TestBench::coeffThroughInvTq_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    // Build a COEFF packet with zeroed qcoeffs (16 int16 -> 32 bytes)
    pkt.cmd = CustomCmd::COEFF;
    pkt.block_idx = 1;
...
bool TestBench::large_block_tq_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
...
bool TestBench::pack_unpack_roundtrip_test(const CustomPacket &pkt_in) {
    CustomPacket p;
    p.cmd = CustomCmd::READ_REQ;
    p.block_idx = 42;
    p.x = 3; p.y = 4;
    ...
    CustomPacket out = unpackCustomPacket(buf.data(), buf.size());
...
bool TestBench::subblock_offset_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    pkt.cmd = CustomCmd::READ_REQ;
    pkt.block_idx = 11;
    ...
    pkt.i4x4_x = 0; pkt.i4x4_y = 0;
    std::vector<uint8_t> buf1 = packCustomPacket(pkt);
    ...
    if (!coeff_monitor.last_data.empty()) {
        out1 = unpackCustomPacket(coeff_monitor.last_data.data(), coeff_monitor.last_data.size());
    } else if (!db_monitor.last_data.empty()) {
        out1 = unpackCustomPacket(db_monitor.last_data.data(), db_monitor.last_data.size());
    } else {
        return false;
    }

    pkt.i4x4_x = 1; pkt.i4x4_y = 1;
    std::vector<uint8_t> buf2 = packCustomPacket(pkt);
    ...
    CustomPacket out2;
    if (!coeff_monitor.last_data.empty()) {
        out2 = unpackCustomPacket(coeff_monitor.last_data.data(), coeff_monitor.last_data.size());
    } else if (!db_monitor.last_data.empty()) {
        out2 = unpackCustomPacket(db_monitor.last_data.data(), db_monitor.last_data.size());
    } else {
...
bool TestBench::qp_extremes_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    pkt.cmd = CustomCmd::RESIDUAL;
    pkt.block_idx = 21;
    ...
    // low qp
    pkt.qp = 0;
    std::vector<uint8_t> buf_low = packCustomPacket(pkt);
    ...
    // high qp
    pkt.qp = 63;
    std::vector<uint8_t> buf_high = packCustomPacket(pkt);
...
bool TestBench::short_payload_handling_test(const CustomPacket &pkt_in) {
    CustomPacket pkt = pkt_in;
    pkt.cmd = CustomCmd::COEFF;
    pkt.block_idx = 99;
    pkt.x = 0; pkt.y = 0; pkt.size = 0; pkt.sel = 0; pkt.qp = 22;
