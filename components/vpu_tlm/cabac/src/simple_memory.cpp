#include "../include/simple_memory.h"

#include "debug_config.h"

CabacSimpleMemory::CabacSimpleMemory(sc_core::sc_module_name name)
    : sc_core::sc_module(name), socket("socket") {
    socket.register_b_transport(this, &CabacSimpleMemory::b_transport);
    load_cabac_tables();
}

void CabacSimpleMemory::load_data(uint64_t addr, const std::vector<uint8_t>& data) {
    uint64_t a = addr;
...
void CabacSimpleMemory::b_transport(tlm::tlm_generic_payload& trans, sc_core::sc_time& delay) {
...
bool CabacSimpleMemory::load_cabac_tables(const std::vector<std::string>& /*candidates*/) {
