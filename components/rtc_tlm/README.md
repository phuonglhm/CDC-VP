...
Memory-mapped TLM-2.0 real-time clock, a memory-mapped real-time clock with alarm.
...
| `0x0C` | CR   | R/W   | bit0 EN (RTCEN): write-once, set-only enable. |
...
- The counter wraps at 2^32 (intentional free-run), a 32-bit
  up-counter.
- `CR.EN` (RTCEN) is write-once: software can start the RTC but cannot stop
  it by writing 0. Only `reset_n` clears it. This matches typical RTC firmware
...
- The alarm latches in `RIS` on the tick where `DR` reaches `MR` (edge), and is
  cleared by `ICR` (W1C) — consistent with the QEMU RTC model.
